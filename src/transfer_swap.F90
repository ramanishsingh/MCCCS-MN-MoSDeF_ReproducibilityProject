module transfer_swap
  ...
  integer,allocatable::bnattempts(:,:,:),bnattempts_nonempty(:,:,:),bsswap(:,:,:),bnswap(:,:,:),bnswap_inin(:,:),bnswap_outin(:,:)&
  ,bnswap_inout(:,:),bnswap_outout(:,:)
...
! *************************
    ...
    if ( .not. lswapinter .and. lbias(imolty) ) then
       if ( lins_in ) then
          if ( lrem_out ) then
             bnswap_outin(imolty,1) = bnswap_outin(imolty,1) + 1
          else
             bnswap_inin(imolty,1) = bnswap_inin(imolty,1) + 1
          end if
       else
          if ( lavbmc3(imolty) ) then
             if ( linsk_in ) then
                ...
                bnswap_inout(imolty,1) = bnswap_inout(imolty,1) + 1
             endif
          else
             if ( lrem_out ) then
                bnswap_outout(imolty,1) = bnswap_outout(imolty,1) + 1
             else
                bnswap_inout(imolty,1) = bnswap_inout(imolty,1) + 1
             endif
          end if
...
! we can now accept !!!!! ***
       ...
       if ( .not. lswapinter .and. lbias(imolty) ) then
          if ( lins_in ) then
             if ( lrem_out ) then
                bnswap_outin(imolty,2) = bnswap_outin(imolty,2) + 1
             else
                bnswap_inin(imolty,2) = bnswap_inin(imolty,2) + 1
             end if
          else 
             if ( lavbmc3(imolty) ) then
                if ( linsk_in ) then
                   ...
                   bnswap_inout(imolty,2) = bnswap_inout(imolty,2) + 1
                endif
             else
                if ( lrem_out ) then
                   bnswap_outout(imolty,2) = bnswap_outout(imolty,2) + 1
                else
                   bnswap_inout(imolty,2) = bnswap_inout(imolty,2) + 1
                endif
             end if
...
! -----------------------------------------------------------------
  ...
  subroutine init_swap(io_input,lprint)
    ...
    if (allocated(acchem)) deallocate(acchem,bnchem,bnattempts,bnattempts_nonempty,bsswap,bnswap,bnswap_inin,bnswap_outin&
     ,bnswap_inout,bnswap_outout,stat=jerr)
    allocate(acchem(nbxmax,ntmax),bnchem(nbxmax,ntmax),bnattempts(ntmax,npabmax,nbxmax),bnattempts_nonempty(ntmax,npabmax,nbxmax)&
     ,bsswap(ntmax,npabmax,nbxmax),bnswap(ntmax,npabmax,nbxmax),bnswap_inin(ntmax,2),bnswap_outin(ntmax,2)&
     ,bnswap_inout(ntmax,2),bnswap_outout(ntmax,2),stat=jerr)
  ...
  subroutine output_swap_stats(io_output)
    ...
    do i = 1, nmolty
       ...
       write(io_output,"('number of in in moves: ', I0,  '  accepted = ',I0)") bnswap_inin(i,1), bnswap_inin(i,2)
       write(io_output,"('number of out in moves: ', I0,  '  accepted = ',I0)") bnswap_outin(i,1), bnswap_outin(i,2)
       write(io_output,"('number of in out moves: ', I0,  '  accepted = ',I0)") bnswap_inout(i,1), bnswap_inout(i,2)
       write(io_output,"('number of out out moves: ', I0,  '  accepted = ',I0)")bnswap_outout(i,1), bnswap_outout(i,2)
    end do
  ...
  subroutine read_checkpoint_swap(io_chkpt)
    ...
    if (myid.eq.rootid) read(io_chkpt) bnattempts,bnattempts_nonempty,bsswap,bnswap,bnchem,acchem&
     ,bnswap_inin,bnswap_outin,bnswap_inout,bnswap_outout
    ...
    call mp_bcast(bnswap_inin,ntmax*2,rootid,groupid)
    call mp_bcast(bnswap_outin,ntmax*2,rootid,groupid)
    call mp_bcast(bnswap_inout,ntmax*2,rootid,groupid)
    call mp_bcast(bnswap_outout,ntmax*2,rootid,groupid)
  end subroutine read_checkpoint_swap

  subroutine write_checkpoint_swap(io_chkpt)
    integer,intent(in)::io_chkpt
    write(io_chkpt) bnattempts,bnattempts_nonempty,bsswap,bnswap,bnchem,acchem&
     ,bnswap_inin,bnswap_outin,bnswap_inout,bnswap_outout
