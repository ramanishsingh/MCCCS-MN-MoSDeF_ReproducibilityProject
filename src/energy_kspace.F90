MODULE energy_kspace
  ...
  use sim_system,only:lsolid,lrect,lrecip,boxlx,boxly,boxlz,nchain,moltyp,lelect,nboxi,ntype,lqchg,rxu,ryu,rzu,qqu,myid,numprocs,moltion&
   ,nunit,rxuion,ryuion,rzuion,qquion,xcm,ycm,zcm,groupid,rcut,rootid,lexclu
  use sim_cell
  implicit none
  private
  save
    ...
    if(lrecip .and. (.not. lsolid(ibox)) .or. lrect(ibox)) then
       ...
       do i = mystart,myend
          ...
          do ii = 1,nunit(imolty)
             if ( lqchg(ntype(imolty,ii)) .and. abs(qqu(i,ii)).gt.1e-6 .and..not. lexclu(imolty,ii,imolty,ii)) then
             ...
             do n = n_min, kmaxn
                ...
                if ( ksqr .lt. hmaxsq .and. abs(ksqr-hmaxsq) .gt. 1E-9_dp ) then
                   ...
                   do i = 1,nchain
                      ...
                      if (.not.lelect(imolty).or.nboxi(i).ne.ibox ) cycle
                      do ii = 1,nunit(imolty)
                         if ( lqchg(ntype(imolty,ii)) .and..not. lexclu(imolty,ii,imolty,ii)) then
...
! new conformation izz = 2 (which is 0 for swap removed molecule)
    ...
    if(lrecip .and. (.not. lsolid(ibox)) .or. lrect(ibox)) then
       ...
       do izz = 1,2
          imolty = moltion(izz)   
          do ii = 1, nunit(imolty)
             if ( lqchg(ntype(imolty,ii)) .and. abs(qquion(ii,izz)).gt.1e-6 .and..not. lexclu(imolty,ii,imolty,ii)) then
