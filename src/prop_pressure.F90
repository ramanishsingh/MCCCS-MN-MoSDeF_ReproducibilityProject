...
!> energy), prop_pressure::pressure (force calculation)
  function corp(imolty,jmolty,rhosq,ibox)
    ...
    do ii = 1, nunit(imolty)
       ...
       do jj = 1, nunit(jmolty)
          ntjj = ntype(jmolty,jj)
          ntij=type_2body(ntii,ntjj)
          ! Cycle if force field pressure calculation isn't supported and check
          ! exclusion table.
          if (lgaro.or.lsami.or.lmuir.or.lexclu(imolty,ii,jmolty,jj)) cycle
          if (nonbond_type(ntij).eq.1) then
             ! LJ 12-6
  ...
  function fij_calculation(i, imolty, ii, j, jmolty, jj, rijsq, rij, ntii, ntjj, ntij) result(fij)
      ...
      if (lgaro.or.lsami.or.lmuir.or.lexclu(imolty,ii,jmolty,jj)) then
