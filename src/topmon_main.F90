...
! -------------------------------------------------------------------
    ...
    do ibox=1,nbox
       ...
       ! need to check
       if (myid.eq.rootid) then
          if ( abs((v(ivTot) - vbox(ivTot,ibox))/vbox(ivTot,ibox)) .gt. 1e-12) then      
                write(io_output,*) '### problem with total energy ###  box ',ibox
                write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
                write(io_output,*) v(ivTot),vbox(ivTot,ibox),abs(v(ivTot)-vbox(ivTot,ibox)),(v(ivTot)-vbox(ivTot,ibox))/abs(vbox(ivTot,ibox))
          end if
           if ( abs((v(ivInterLJ) - vbox(ivInterLJ,ibox))/vbox(ivInterLJ,ibox)).gt. 1e-12) then  
             write(io_output,*) '### problem with inter mol.en.  ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivInterLJ),vbox(ivInterLJ,ibox),abs(v(ivInterLJ)-vbox(ivInterLJ,ibox)),(v(ivInterLJ)-vbox(ivInterLJ,ibox))/abs(vbox(ivInterLJ,ibox))
             if (lsolid(ibox) .and. .not. lrect(ibox)) then
                write(io_output,*)'You might check the cutoff wrt box widths'
                write(io_output,*) 'Normal PBC might be failing'
             end if
          end if
          if ( abs((v(ivTail) - vbox(ivTail,ibox))/vbox(ivTail,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem  with tail corr.en. ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivTail),vbox(ivTail,ibox),(v(ivTail) - vbox(ivTail,ibox)),(v(ivTail) - vbox(ivTail,ibox))/abs(vbox(ivTail,ibox))
          end if
          if ( abs((v(ivIntraLJ) - vbox(ivIntraLJ,ibox))/vbox(ivIntraLJ,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem with intra mol.en.  ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivIntraLJ),vbox(ivIntraLJ,ibox),(v(ivIntraLJ) - vbox(ivIntraLJ,ibox)),(v(ivIntraLJ) - vbox(ivIntraLJ,ibox))/abs(vbox(ivIntraLJ,ibox))
          end if
          if ( abs((v(ivStretching) - vbox(ivStretching,ibox))/vbox(ivStretching,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem with bond vib.en. ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivStretching),vbox(ivStretching,ibox),(v(ivStretching) - vbox(ivStretching,ibox)), (v(ivStretching) - vbox(ivStretching,ibox))/abs(vbox(ivStretching,ibox))
          end if
          if ( abs((v(ivBending) - vbox(ivBending,ibox))/vbox(ivBending,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem with bond ben.en.  ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) ' Bond ben.en.:',v(ivBending),vbox(ivBending,ibox),(v(ivBending) - vbox(ivBending,ibox)),(v(ivBending) - vbox(ivBending,ibox))/abs(vbox(ivBending,ibox))
          end if
          if ( abs((v(ivTorsion) - vbox(ivTorsion,ibox))/vbox(ivTorsion,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem with torsion.en.  ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivTorsion),vbox(ivTorsion,ibox),(v(ivTorsion) - vbox(ivTorsion,ibox)),(v(ivTorsion) - vbox(ivTorsion,ibox))/abs(vbox(ivTorsion,ibox))
          end if
          if ( abs((v(ivExt) - vbox(ivExt,ibox))/vbox(ivExt,ibox)) .gt. 1e-12)then
             write(io_output,*) '### problem with Externa.en. ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivExt),vbox(ivExt,ibox),(v(ivExt) - vbox(ivExt,ibox)),(v(ivExt) - vbox(ivExt,ibox))/abs(vbox(ivExt,ibox))
          end if
          if ( abs((v(ivElect) - vbox(ivElect,ibox))/vbox(ivElect,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem with Coulomb.en. ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivElect),vbox(ivElect,ibox),(v(ivElect) - vbox(ivElect,ibox)), (v(ivElect) - vbox(ivElect,ibox))/ abs(vbox(ivElect,ibox)) 
         end if
          if ( abs((v(ivFlucq) - vbox(ivFlucq,ibox))/vbox(ivFlucq,ibox)) .gt. 1e-12) then
             write(io_output,*) '### problem with Fluc Q en.  ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivFlucq),vbox(ivFlucq,ibox),(v(ivFlucq) - vbox(ivFlucq,ibox)), (v(ivFlucq) - vbox(ivFlucq,ibox))/abs(vbox(ivFlucq,ibox))
          end if
          if ( abs((v(iv3body) - vbox(iv3body,ibox))/ vbox(iv3body,ibox) ) .gt. 1e-12) then
             write(io_output,*) '### problem with 3-body en. ###'
             write(io_output,*) 'summup, Markov chain, abs err, rel_err:'
             write(io_output,*) v(iv3body),vbox(iv3body,ibox),v(iv3body) -  vbox(iv3body,ibox),(v(iv3body) - vbox(iv3body,ibox))/abs(vbox(iv3body,ibox))
          end if
          if ( ldielect ) then
             if ( abs((dipolexo - dipolex(ibox))/ dipolex(ibox)) .gt. 1e-12)  then
                write(io_output,*) '### problem with dipole x ###'
                write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
                write(io_output,*) ' Dipole X:',dipolexo,dipolex(ibox),(dipolexo - dipolex(ibox)), (dipolexo - dipolex(ibox))/abs(dipolex(ibox))
             end if
          end if
          if (lmipsw) then
                if(abs((vwellipsw-vbox(ivWellIpswb,ibox))/vbox(ivWellIpswb,ibox)).gt. 1e-12) then
                write(io_output,*) '### problem with well en.  ###'
                write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
                write(io_output,*) vwellipsw,vbox(ivWellIpswb,ibox),(vwellipsw-vbox(ivWellIpswb,ibox)),(vwellipsw-vbox(ivWellIpswb,ibox))/abs(vbox(ivWellIpswb,ibox))
             end if
          end if
       end if
    end do
...
! ===================================================================
    ...
    if (linit) then
       ...
       do i=1,nbox
          if (i.eq.1.and.(lexzeo.or.(lionic.and..not.lpbc))) then ! CK Chang changed to avoid bypassing the neutrality test for lionic=T PBC systems.
             cycle
