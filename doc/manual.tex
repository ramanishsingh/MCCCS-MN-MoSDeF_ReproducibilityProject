...
% -updated by TRJ and others, 03/20 -- version 2.6
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LaTeX controls:
...
\centerline{\bf \Large Version 2.6}

\vskip 36pt

\centerline{\large March 2020}
...
\noindent{\bf lmixwh:} If TRUE, the Walderman-Hagler combining
rules for the for the Lennard-Jones potential will be used.

\noindent{\bf lmixkong:} If TRUE, the Kong combining
rules for the for the Lennard-Jones potential will be used.
This mixing rule is recommended by Mansi Shah in her ethane-
ethylene paper for subsequent force field development.
...
\noindent{\bf epsi:} Energy related to temperature via
the Boltzmann's constant. $\epsilon/|k_{B}$

\noindent{\bf sigi:} Point at which particle potential is 0.
...
ATOMS setup of ethanol from TRaPPE data on Siepmann Group website.
\pagebreak
\begin{verbatim}
ATOMS
!i type(i) epsi(i) sigi(i) q(i)     mass(i) chemid(i) chname(i) [Angstrom, Kelvin, a.u.]; type 1: Lennard-Jones
1   1       98.0    3.75   0.0      15.035   CH3
2   1       46.0    3.95   0.265    14.027   CH2
3   1       93.0    3.020  -0.700   15.999   O
4   1       0.0     0.0    0.435    1.008    H
END ATOMS
...
your equilibrium bond length where the standard deviation
...
\noindent {\bf Example \#1}\hfill\break
BONDS setup of ethanol from TRaPPE data on Siepmann Group website.

\begin{verbatim}
BONDS
!i type(i) brvib(i) brvibk(i) [Angstrom]; type 1: U = brvibk * ( r_ij - brvib ) ^ 2
1   1       1.54  0.0  #CHx-CHy
2   1       1.43  0.0  #CHx-OH
3   1       0.945 0.0  #O-H
END BONDS
\end{verbatim}
...
\noindent {\bf Example \#1}\hfill\break
ANGLES setup of ethanol from TRaPPE data on Siepmann Group website.

\begin{verbatim}
ANGLES
!i type(i) brben(i) brbenk(i) [degree]; type 1: U = brbenk * ( theta_ijk - brben ) ^ 2 [radian]
1   1       109.5     25200  #CHx-(CHy)-OH
2   1       108.5     27700  #C-O-H
END ANGLES
\end{verbatim}
...
\noindent {\bf Example \#1}\hfill\break
DIHEDRALS setup of ethanol from TRaPPE data on Siepmann Group website.

\begin{verbatim}
DIHEDRALS
!i type(i) vtt0(i) vtt1(i) vtt2(i) vtt3(i); !!!check vtorso.f, calctor.f!!!
1    1     0.0      209.82   -29.17  187.93 #C-CH2-O-H
END DIHEDRALS
...
volume move. Typically set to 1 divided by {\bf tavol} times {\bf nchain} to
...
\subsection{Section \textbf{ZEOLITE\_EXCLUSION}}
Special rules for exclusion of all interactions of a molecule
type with an external potential (e.g., a zeolite). This is
useful for silanol groups on a nanosheet, where it is desired
to represent the silanol groups explicitly to have dancing
protons. By default, all molecule types have interaction
with an external potential. Each line specifies one molecule
for which it is desired to remove interactions
with an external potential. This section is
optional (has backwards compatibility). For example, if
molecule types 1 and 2 are silanol groups, one might
want to add the following to the input file:
\begin{verbatim}
ZEOLITE_EXCLUSION
1
2
END ZEOLITE_EXCLUSION
\end{verbatim}
...
\noindent{\bf file\_traj:} Contains the box lengths (in angstroms), total
energy (in K), pressure (in kPa), and number of each molecule type for each
...
basic information as to what is there in that particular
directory.
...
the critical point, so we set {\bf pmvol} to 1 / ({\bf tavol} * {\bf nchain})
as we will be going for a target acceptance ratio of 30\% for
volume moves ({\bf tavol} = 0.3). We set {\bf iratv} so that the maximum
